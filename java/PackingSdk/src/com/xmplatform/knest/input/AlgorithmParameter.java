...
public class AlgorithmParameter {
    private byte nestType = 0;                           //嵌套计算的类型，0 表示不嵌套，1 表示简单异形嵌套，2 表示复杂异形嵌套
    private byte baseCorner = 1;                         //优化的堆积角落，1表示左下角，2表示右下角，3表示右上角，4表示左上角
    private boolean enablePriority = false;              //是否启用板块的排版优先级（优先级的值默认是0）
    private byte priorityType = 1;                       //优先级类型 1表示优先级大于0的尽量靠前，2表示优先级大于0的绝对靠前
    private boolean higherPriorityPlacedFront = false;   //大板中的小板存在优先级定义时，该大板靠前还是靠后, true 则放在前面，false 则放在后面
    private int resultTimeOut = -1;                      //得到优化结果后，等待多长时间内(毫秒)再无优化结果变化，则算法停止运行，当为-1表示不限制，即一直等待
    private boolean jsonResultAsFile = false;            //是否将输出结果导出为文件。默认值为false，false表示NestOutput的jsonResult属性中放入的是json字符串(NestOutput)，true表示将NestOutput对应的json保存为文件，并且将该json文件路径放入到jsonResult中
