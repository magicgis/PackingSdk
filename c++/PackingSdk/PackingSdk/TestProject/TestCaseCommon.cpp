...
// (usb加密狗)适用的测试用例
namespace knest {
    const int DOG_TYPE = 1;//狗类型为USB加密狗
    //设置日志级别
    TEST(Common, setLogLevel) {
        //设置日志级别
        setLogLevel(2);
        helloWorld();
    }
    //获取加密狗code
    TEST(Common, getDogCode) {
        //获取加密狗的code
    ...
    //根据加密狗类型获取加密狗code
    TEST(Common, getDogCodeByDogType) {
        //获取usb加密狗的code
    ...
    //根据加密狗类型获取加密狗code2
    TEST(Common, getDogCodeByDogType2) {
        //获取usb加密狗的code
        char* dogCode = nullptr;
